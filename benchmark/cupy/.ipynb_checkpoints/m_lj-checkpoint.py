from cupy import zeros, ones, rint, sqrt, sum, pi,amax
from cupy import random,append
from cupy import int32,float64
...
class LJ :
  ...
  def calcener(self, mx, my, mz, N ) :
    ...
    f1z= zeros(N)
    #
    nmax = int(27*amax(self.np)) ### cupy likes this way
    rc1x = zeros(nmax)
    rc1y = zeros(nmax)
    rc1z = zeros(nmax)
    # Loop over Cells
    for vcx in range(mx):
        for vcy in range(1,my-1):
            for vcz in range(1,mz-1):
                ...
                int_npc1 = int(self.indp[c +1])
                int_npc = int(self.indp[c])
                for i in range(int_npc,int_npc1): ### cupy likes this way
                    #
